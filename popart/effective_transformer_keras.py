...
class EffectiveTransformer(Layer):
  ...
  Arguments:
    ...
    max_batch_size: The upper limit to which additional sequences will
      be processed per pass due to sequence compression.
    ...
    sequences_per_iter: The number of full-sequence equivelants to process.
  ...
  def build(self, input_shapes):
    ...
    self._sequence_length_buffer = array_ops.zeros((self.max_batch_size, 1),
                                                   dtype=dtypes.int32)
    self._sequence_length_buffer = array_ops.stop_gradient(
        self._sequence_length_buffer)

    # Buffers for padded q, k and v.
    self._padded_q_buffer = array_ops.zeros(
        (self.max_batch_size,
         from_dim * self.num_attention_heads * self.attention_head_size),
        dtype=self._q_layer.dtype)

    self._padded_k_buffer = array_ops.zeros(
        (self.max_batch_size,
         to_dim * self.num_attention_heads * self.attention_head_size),
  ...
  def call(self,
    ...
    if not self.built:
      raise RuntimeError(
          "EffectiveTransformer must be built before being called.")

    from_sequences = ops.convert_to_tensor(from_sequences)
    to_sequences = ops.convert_to_tensor(to_sequences)
    from_sequence_lengths = ops.convert_to_tensor(from_sequence_lengths)
    to_sequence_lengths = ops.convert_to_tensor(to_sequence_lengths)

    from_sequence_lengths = array_ops.stop_gradient(from_sequence_lengths)
    to_sequence_lengths = array_ops.stop_gradient(to_sequence_lengths)
    ...
    q_mask = array_ops.stop_gradient(q_mask)
    ...
    context = loops.while_loop(partial(self.__class__._main_loop_condition,
                                       self),
                               partial(self.__class__._main_loop_body, self),
                               inputs=[
                                   context, from_sequences,
                                   from_sequence_lengths, to_sequences,
  ...
  def _main_loop_body(self, context, from_sequences, from_sequence_lengths,
    ...
    min_idx = array_ops.stop_gradient(min_idx)
    max_idx = array_ops.stop_gradient(max_idx)
  ...
  def _get_compressed_sequence_bounds(self, from_seq_lengths, to_seq_lengths,
    ...
    res = loops.while_loop(_loop_condition,
                           _loop_body,
                           inputs=[idx, 0, 0],
  ...
  def _slice_out_sequences(self, sequences, min_idx, max_idx, buffer_size):
    ...
    if dtype not in [dtypes.float16, dtypes.float32, dtypes.int32]:
      dtype = dtypes.float32
  ...
  def _unpad(self, sequences, sequence_lengths, slices_per_iter):
    ...
    num_written = array_ops.stop_gradient(num_written)

    out_row_idx = array_ops.zeros((), dtype=dtypes.int32)
    out_row_idx = array_ops.stop_gradient(out_row_idx)

    slice_no = array_ops.zeros((), dtype=dtypes.int32)
    slice_no = array_ops.stop_gradient(slice_no)

    sequence_lengths = array_ops.stop_gradient(sequence_lengths)

    res = loops.repeat(slices_per_iter,
                       _loop_body,
                       inputs=[
  ...
  def _pad_embedding(self, embedding, sequence_lengths, min_idx, max_idx,
    ...
    dst_offsets = math_ops.range(self.max_batch_size)
